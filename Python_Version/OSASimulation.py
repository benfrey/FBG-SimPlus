...
class OSASimulation(object):
    ...
    def TransverseStrain(self,FBGOriginalWavel,PhotoElasticParam,InitialRefractiveIndex,MeanChangeRefractiveIndex,FringeVisibility,MinBandWidth,MaxBandWidth,SimulationResolution,FBGDirection,DirectionalRefractiveP11,DirectionalRefractiveP12,YoungsModule,PoissionsCoefficient):
        ...
        for i in np.arange(0,self.NumberFBG):
            ...
            print("Dy: "+str(self.Dneffy))
            print("Dz: "+str(self.Dneffz))
    ...
    def Temperature(self,FBGOriginalWavel,PhotoElasticParam,InitialRefractiveIndex,MeanChangeRefractiveIndex,FringeVisibility,MinBandWidth,MaxBandWidth,SimulationResolution,FBGDirection,ThermalExpansion,ThermoOptic,Temperature):
        """
        Function to Simulate the FGB Spectrum Considering only Uniform Strain
        Contribution
        ...
        FBGDirection: int (mandatory)
                    0-xx 1-yy 2-zz
        ThermalExpansion: float (Mandatory)
                        Thermal expansion coefficient of fiber
        ThermoOptic: float (Mandatory)
                        Thermo-optic coefficient of fiber
        Temperature: int (Mandatory)
        ...
        self.APFBG=[]
        for i in np.arange(0,self.NumberFBG):
            TempnewWavelength=self.FBGOriginalWavel[i]*(1+(self.ThermalExpansion+self.ThermoOptic)*(self.Temperature-self.To)) #weavelength due only to temperature
            self.APFBG.append(TempnewWavelength/(2.0*self.InitialRefractiveIndex)) # Grating period at strain state

        #print(self.APFBG)
        #Empty Reflec spectrum - Only temperature
        self.TReflect={}
        self.TReflect['wavelength']=[]
        self.TReflect['reflec']=[]
        ...
        for i in np.arange(0,self.NumberFBG):
            ...
            for l in np.arange(self.MinBandWidth,self.MaxBandWidth,self.SimulationResolution):
                ...
                NO=f1[1,0]
                REF=abs(NO/PO)**2
                self.TReflect['wavelength'].append(l)
                self.TReflect['reflec'].append(REF)



    def TransverseStrainTemperature(self,FBGOriginalWavel,PhotoElasticParam,InitialRefractiveIndex,MeanChangeRefractiveIndex,FringeVisibility,MinBandWidth,MaxBandWidth,SimulationResolution,FBGDirection,DirectionalRefractiveP11,DirectionalRefractiveP12,YoungsModule,PoissionsCoefficient,ThermalExpansion,ThermoOptic,Temperature):
        """
        Function to Simulate the FGB Spectrum Considering Uniform longitudinal strain,
        transverse Stress, and temperature
        ...
        for i in np.arange(0,self.NumberFBG):
            if self.FBGDirection==0: #FBG longitudinal direction (xx)
                TempMeanFBG=np.mean(self.FBGArray['FBG'+str(i+1)]['LE11'] )#Strain average
                TempnewWavelength=self.FBGOriginalWavel[i]*(1+(1-self.PhotoElasticParam)*TempMeanFBG+(self.ThermalExpansion+self.ThermoOptic)*(self.Temperature-self.To)) #weavelength at uniform strain and temperature
                self.APFBG.append(TempnewWavelength/(2.0*self.InitialRefractiveIndex)) # Grating period at strain state
        ...
        #Empty Reflec spectrum- Two waves
        self.TSTYReflect={}#Y wave
        self.TSTYReflect['wavelength']=[]
        self.TSTYReflect['reflec']=[]

        self.TSTZReflect={}#Z wave
        self.TSTZReflect['wavelength']=[]
        self.TSTZReflect['reflec']=[]
        ...
        for i in np.arange(0,self.NumberFBG):
           ...
           # Wavelength cycle (Here the simulation resolution is used)
            ...
            for l in np.arange(self.MinBandWidth,self.MaxBandWidth,self.SimulationResolution):
                ...
                NO=f1[1,0]
                REF=abs(NO/PO)**2
                self.TSTYReflect['wavelength'].append(l) #Output File
                self.TSTYReflect['reflec'].append(REF) #Output File
            ...
            for l in np.arange(self.MinBandWidth,self.MaxBandWidth,self.SimulationResolution):
                ...
                NO=f1[1,0]
                REF=abs(NO/PO)**2
                self.TSTZReflect['wavelength'].append(l) #Output File
                self.TSTZReflect['reflec'].append(REF)  #Output File



    def FBGOutputSum(self,FBGOriginalWavel,PhotoElasticParam,InitialRefractiveIndex,FBGDirection,DirectionalRefractiveP11,DirectionalRefractiveP12,YoungsModule,PoissionsCoefficient,ThermalExpansion,ThermoOptic,Temperature):
        """
        ...
        for b in np.arange(0,self.NumberFBG): #Cycle each FBG
            ...
            WavlShift=((1.0-self.PhotoElasticParam)*FBGmaxmin['FBG'+str(b+1)][AVStrainDirection]+(self.ThermalExpansion+self.ThermoOptic)*(self.Temperature-self.To))*self.FBGOriginalWavel[b]
            ...
            graperiodmax=self.FBGOriginalWavel[b]/(2*self.InitialRefractiveIndex)*(1+(1-self.PhotoElasticParam)*FBGmaxmin['FBG'+str(b+1)][MaxStrainDirection]+(self.ThermalExpansion+self.ThermoOptic)*(self.Temperature-self.To))
            #Grating period caused by minimum strain
            graperiodmin=self.FBGOriginalWavel[b]/(2*self.InitialRefractiveIndex)*(1+(1-self.PhotoElasticParam)*FBGmaxmin['FBG'+str(b+1)][MinStrainDirection]+(self.ThermalExpansion+self.ThermoOptic)*(self.Temperature-self.To))
